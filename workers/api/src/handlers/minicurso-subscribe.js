// Cloudflare Worker handler: Mini-Course Subscribe + Schedule All Emails
// POST /api/minicurso/subscribe
// Uses Resend API with scheduled_at for drip delivery (Days 2-5)

import { jsonResponse } from '../cors.js';
import { readJsonBody } from '../resend.js';
...
export async function handle(req, env) {
  ...
  if (!RESEND_API_KEY) return jsonResponse({ error: 'RESEND_API_KEY missing' }, { status: 500 }, origin);
  ...
  if (!name || !email || !course_id) {
    return jsonResponse({ error: 'Campos obrigatorios: name, email, course_id' }, { status: 400 }, origin);
  }
  const config = COURSE_CONFIG[course_id];
  if (!config) {
    return jsonResponse({ error: 'course_id invalido. Opcoes: ia-negocios, financas-pessoais, renda-extra' }, { status: 400 }, origin);
  }
  const emails = getEmails(course_id, name);
  if (!emails) return jsonResponse({ error: 'Erro ao gerar emails do curso' }, { status: 500 }, origin);

  try {
    // 1. Adiciona contato na audience (silencia erros — segue mesmo se falhar)
    try {
      ...
      console.error('Erro audience add:', audienceErr.message);
    }

    // 2. Envia Dia 1 imediato
    const day1 = emails[0];
    ...
    if (!day1Res.ok) {
      ...
      console.error('Erro Dia 1:', errBody);
      return jsonResponse({ error: 'Erro ao enviar email do Dia 1' }, { status: 500 }, origin);
    }

    // 3. Agenda Days 2-N com scheduled_at
    const now = Date.now();
    ...
    for (let i = 1; i < emails.length; i++) {
      ...
      try {
        ...
        scheduleResults.push({ day: emailData.day, scheduled_at: scheduledAt, success: schedRes.ok, id: schedBody.id || null });
      } catch (schedErr) {
        scheduleResults.push({ day: emailData.day, scheduled_at: scheduledAt, success: false, error: schedErr.message });
      }
    }
    ...
    console.error('Erro geral minicurso:', err.message);
    return jsonResponse({ error: 'Erro interno do servidor' }, { status: 500 }, origin);
  }
}