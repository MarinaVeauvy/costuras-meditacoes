# Bios Prontas — Perfis Midas

> **Estratégia atual (29/04/2026):** posts intercalam versículos (neutros) com cortes MAC.
> Bios reposicionadas pra tom espiritual/família — tema cripto/investir queimou algoritmo das contas zeradas.
> Bios v1 (cripto direto) marcadas como DEPRECATED abaixo.

**Evitar nas bios:** "cripto", "investir", "lucro", "ganhe", valores específicos, "garantido", "sem risco", emojis $$, sinais matemáticos, categoria Finance/Investimentos.

**Categoria preferida:** Educação · Estilo de vida · Família. NUNCA Finance.

---

## 1. @pros.peridadedoreino (atual)

**Persona:** Fé + prosperidade bíblica aplicada a cripto
**Público:** Mulheres evangélicas 28-50 curiosas sobre investimentos

### Bio Instagram (150 caracteres)
```
✝️ Prosperidade do Reino
📊 Mercado cripto explicado com fé
👩 Mulher comum aprendendo a investir
👇 Método completo
```

### Bio TikTok
```
✝️ Prosperidade do Reino
📊 Cripto pra cristão
👇 Link na bio
```

### Bio YouTube
```
Prosperidade do Reino — conteúdo sobre mercado de criptomoedas explicado de forma acessível pra mulheres cristãs. Aqui você vai entender o que é blockchain, como funciona Bitcoin, e conhecer um método educativo para dar os primeiros passos no mercado digital.

📌 Conteúdo educativo. Decisões de investimento são pessoais.
```

---

## 2. @orarprosperar — ATIVA (TikTok aplicado 29/04/2026)

**Persona:** Mãe de fé + propósito
**Público:** Mulheres 30-45, evangélicas, foco família/casa
**Display name TikTok:** `Orar e Prosperar`

### Bio TikTok (ATIVA — Opção A neutra)
```
🙏 Mãe + fé + propósito
📖 Provérbios na prática
✨ IG: @orarprosperar
```
65 caracteres · zero gatilho cripto · alinhada com versículos do pipeline

### Bio Instagram (ATIVA — neutra)
```
🙏 Orar + prosperar
📖 Versículos para o dia a dia
👩‍👧 Mãe + fé + propósito
✨ Conteúdo educativo
```

### Bio YouTube (ATIVA — neutra)
```
Conteúdo de fé, família e propósito para mães que buscam viver com mais leveza, propósito e equilíbrio financeiro. Versículos, reflexões e dicas práticas pra dia a dia.

📌 Conteúdo educativo.
```

### Bio TikTok v2 (futura — quando bater 500+ followers)
```
👨‍👩‍👧 Família próspera com Deus
📚 Educação financeira do dia a dia
✨ IG: @orarprosperar
```
77 chars · introduz tema dinheiro envolto em fé/família

---

### ⚠️ DEPRECATED v1 (cripto direto, queimou algoritmo)

```
🙏 Orar + prosperar
👩‍👧 Mãe aprendendo mercado cripto
📚 Educação financeira sem enrolação
📌 Método completo 👇
```
Não usar — tema cripto direto suprimiu reach das contas novas.

---

## 3. @liberdade.com.fe (criar)

**Persona:** Transformação + estudo de cripto
**Público:** Mulheres 25-45 buscando alternativas ao mercado tradicional

### Bio Instagram
```
🕊️ Liberdade com fé
📈 Estudando o mercado cripto
💪 Conhecimento é liberdade
✨ Método educativo 👇
```

### Bio TikTok
```
🕊️ Estudando cripto com fé
📈 Conhecimento = liberdade
👇 Link
```

### Bio YouTube
```
Testemunho de mulheres que estão se educando sobre o mercado de criptomoedas como forma de conhecer alternativas financeiras. Conteúdo focado em educação, conceitos básicos e fundamentos — não é recomendação de investimento.

📌 Método completo abaixo.
```

---

## Padrão técnico pra todas

### Link na bio → novavidaprospera.com.br

Todas as 3 contas apontam pro mesmo domínio bridge. Tracking via UTM:
- @pros.peridadedoreino → novavidaprospera.com.br/?ref=reino
- @orar.prosperar → novavidaprospera.com.br/?ref=orar
- @liberdade.com.fe → novavidaprospera.com.br/?ref=liberdade

### Foto de perfil (evitar)
- ❌ Foto pessoal (nenhuma é a Marina)
- ❌ Gráficos de trading explícitos (gatilho)
- ❌ Dinheiro em foto

### Foto de perfil (preferir)
- ✅ Imagem abstrata: luz, mandala, cruz estilizada
- ✅ Paleta dourado + branco + azul-noturno
- ✅ Pode usar AI (Midjourney, DALL-E)

### Destaques IG (stories fixos)
Cada conta: 4-6 destaques
- "Começa aqui" — introdução ao mercado cripto
- "Conceitos" — blockchain, Bitcoin, wallets
- "Dúvidas" — FAQ
- "Jornada" — história da persona

### DISCLAIMER obrigatório em pinned post
Cada conta precisa ter 1 post fixo com aviso:

```
📌 Este perfil é educativo.
🚫 Não é recomendação de investimento.
🧠 Decisões financeiras são pessoais — estude e consulte profissional qualificado.
⚠️ Mercado cripto tem riscos. Retorno passado não garante futuro.
```

Isso protege contra:
- Ban por propaganda enganosa
- Processos de consumidor insatisfeito
- Escalation no Reclame Aqui
